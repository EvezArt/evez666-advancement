...
def get_cron_status():
    ...
    try:
        with open('/root/.openclaw/workspace/cron.json', 'r') as f:
            data = json.load(f)
    except Exception as e:
    ...
    for job in data.get('jobs', []):
        ...
        if last_run_at_ms:
            ...
            dt = datetime.fromtimestamp(last_run_at_ms / 1000, tz=timezone.utc)
            time_str = dt.strftime('%Y-%m-%d %H:%M:%S UTC')
        else:
            time_str = 'unknown'
        lines.append(f"- {name}: {last_run_status} (last run: {time_str})")
    return "\n".join(lines)
...
def get_revenue_states():
    ...
    for f in glob.glob(f'{money_dir}/*.json'):
        ...
        try:
            with open(f, 'r') as fp:
                data = json.load(fp)
            info.append(f"{f.split('/')[-1]}: {json.dumps(data, indent=2)}")
        except Exception:
